...
class AntEnv(mujoco_env.MujocoEnv, utils.EzPickle): 
    ...
    def __init__(self, task='walk', latency=0, xyz_noise_std=0.0, rpy_noise_std=0.0, min_obs_stack=1):
        ...
        mujoco_env.MujocoEnv.__init__(self, f'{dir_path}/mujoco.xml', 5,
            observation_space = Box(low=-np.inf, high=np.inf, shape=(29 * self.n_past_obs ,) , dtype=np.float64))
    ...
    def copy_state(self, other_env):
        qpos = other_env.data.qpos
        qvel = other_env.data.qvel
        self.set_state(qpos, qvel)
