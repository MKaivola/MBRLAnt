...
# The issue lies somewhere here in the model or its methods
...
class NPG_Model(nn.Module):
    ...
    # def reset_model(self, ensemble_size, input_size, output_size, hidden_size, device):
    #      self.weights = [trunc_initializer((ensemble_size, input_size, hidden_size), 1.0 / (2.0 * np.sqrt(input_size))).to(device),
    #             trunc_initializer((ensemble_size, hidden_size, hidden_size), 1.0 / (2.0 * np.sqrt(hidden_size))).to(device),
    #             trunc_initializer((ensemble_size, hidden_size, output_size),  1.0 / (2.0 * np.sqrt(hidden_size))).to(device)]
    #      for w in self.weights:
    ...
    def mean_prediction(self, state, act):
        ...
        act = torch.from_numpy(act).float().to(self.device).expand(self.ensemble_size, 1, -1)
        with torch.no_grad():
            state_diffs = self.forward(state, act).mean(0)
            
        prediction = state_diffs * (self.sigma_target + 1e-8) + self.mu_target
        prediction = prediction * self.mask
        
        return self.env_spec.state_postproc(state[0, :, :], prediction).squeeze(0).cpu().numpy()
    ...
    def update_parameters(self, n_epochs = 100, batch_size = 200, min_grad_upds = 10**2,
        ...
        sigma_targets = np.mean(np.abs(targets - mu_targets), axis = 0, keepdims = True) #np.std(targets, axis = 0, keepdims = True)
        # sigma_targets[sigma_targets < 1e-12] = 1.0
        ...
        sigma = np.mean(np.abs(inputs - mu), axis = 0, keepdims = True) #np.std(inputs, axis = 0, keepdims = True)
