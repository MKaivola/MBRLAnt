...
class ForwardNet(nn.Module):
    ...
    def state_descaler(self, state):
        scaled = state * (self.sigma_target + 1e-8)
    ...
    def update_parameters(self, n_epochs = 5, batch_size = 32):
        ...
        for i_epoch in range(n_epochs):
            ...
            for batch_n in range(int(np.ceil(n_train / batch_size))):
                ...
                targets_batch = (targets_batch - self.mu_target)/(self.sigma_target + 1e-8)
...
class DETNET():
    ...
    def save_transition(self, state, action, next_state):
