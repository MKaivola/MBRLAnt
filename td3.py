...
class TD3:
    ...
    def save_model(self, path = 'td3_model.pt'):
        torch.save(self.get_model_param(),
                   path)
        
    def load_model(self, path = 'td3_model.pt'):
    ...
    def act(self, state, train=True, noise = None):
        action = self.actor.act(state, self.device)
        if train:
            action = (
                action + np.random.normal(0, self.exploration_noise if noise is None else noise, size=self.act_size)
